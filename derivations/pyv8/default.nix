...
, ensureNewerSourcesForZipFilesHook, update-python-libraries
, darwin, linuxPackages, llvmPackages, system, which }:
...
let
  ...
  darwinDeps = lib.optionals stdenv.isDarwin (with darwin; [
    cctools
    dtrace
  ]);

  boost = python2.pkgs.boost.override {
    inherit stdenv;
  };

in with python2.pkgs; toPythonModule (stdenv.mkDerivation rec {
    name = "python2.7-PyV8";
    version = "1.0.dev";
    src = fetchFromGitHub {
      owner = "pebble";
